...
### 1.4 MCP Resources（只读数据访问）

> 源码位置：`参考代码/claude-code/src/services/mcp/types.ts` - Resource 相关类型

Resources 是 MCP 服务器提供的**只读数据访问接口**，与 Tools 的本质区别：

| 对比维度 | Tools | Resources |
|----------|-------|----------|
| **交互方式** | 主动调用执行操作 | 被动读取数据 |
| **修改性** | 可修改状态 | 仅读取，不修改 |
| **用途** | 执行任务 | 获取参考信息 |
| **示例** | 创建文件、发送消息 | 获取文件内容、查询数据 |
...
### 1.5 MCP Prompts（模板提示）

> 源码位置：`参考代码/claude-code/src/services/mcp/types.ts` - Prompt 相关类型

Prompts 是 MCP 服务器提供的**预设模板**，客户端可以调用并传入参数生成最终提示。

| 对比维度 | Tools | Prompts |
|----------|-------|----------|
| **交互方式** | 执行函数返回结果 | 生成文本模板 |
| **参数化** | 函数参数 | 模板变量 |
| **输出** | 结构化数据 | 文本（可作为 LLM 输入） |
| **用途** | 执行操作 | 生成提示词 |
...
### 1.6 Sampling（LLM生成请求）

> 源码位置：`参考代码/claude-code/src/services/mcp/types.ts` - Sampling 相关类型

Sampling 是 MCP 协议中允许**服务器向客户端发送 LLM 生成请求**的能力，实现服务器驱动的上下文补充。

#### 能力声明

```json
// 服务器在初始化时声明 Sampling 能力
{
  "capabilities": {
    "sampling": {
...
| 场景 | 说明 |
|------|------|
| **智能路由** | 服务器根据上下文智能选择处理方式 |
| **动态提示生成** | 服务器生成适合当前任务的提示 |
| **上下文补充** | 服务器请求 LLM 补充相关信息 |
| **自动化决策** | 服务器请求 LLM 进行决策判断 |

#### 安全考虑

Sampling 能力允许服务器发起 LLM 请求，需要注意：

1. **权限控制**：Sampling 请求需要用户授权
2. **上下文限制**：防止无限循环或过大请求
3. **结果验证**：客户端应验证返回结果的合理性
...
- [[18-01-📚-MCP系统]]（当前模块）
- [[../02-Tool系统/02-01-🔧-Tool系统]] - MCP工具最终通过Tool系统暴露
- [[../03-权限系统/03-01-🔐-权限系统]] - MCP操作需要权限检查
- [[../04-Hook系统/04-01-🪝-Hook系统]] - Elicitation可在Hook中拦截处理
- [[../08-Skill系统/08-01-📚-Skill系统]] - OMC Skill系统使用MCP作为扩展机制

> [!note]- 关联知识
> [[../02-Tool系统/02-01-🔧-Tool系统]] - MCP通过Tool系统暴露工具能力
> [[../08-Skill系统/08-01-📚-Skill系统]] - OMC Skill利用MCP协议实现扩展
